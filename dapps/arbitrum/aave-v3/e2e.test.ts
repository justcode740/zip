...
import GMX from "../dapps/arbitrum/gmx/gmx";
const {erc20Contract} = require("../scripts/contracts");
...
// supply 10 usdc from eth to aave v3 arb
async function test1(){
    ...
    async function logAccountBalances(address: string) {
        for (const account of accounts) {
            console.log(`$0xsuperanony has ${(await new erc20Contract(address, provider).balanceOf(account)) / 1e18} FRAX`);
        }
    }
    ...
    const fraxbalance = await new erc20Contract(FRAX.address, provider).balanceOf(accounts[0])
    ...
    const bytes = abi.encode(
        ...
        [
            ...
            [
                destination.usdc.address, // axlUSDC
                USDC_ARB.address // USDC (Arb1)
            ]
        ]
    )
    ...
    while (true) {
        const updatedBalance = await new erc20Contract(FRAX.address, provider).balanceOf(accounts[0]);
